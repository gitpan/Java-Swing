package Java::Swing::TreeExpansionListener;
use strict; use warnings;

use Inline Java      => 'STUDY',
           AUTOSTUDY => 1,
           STUDY     => ['PerlTreeExpansionListener'];
...
sub connect {
    ...
    my $listener       =
        Java::Swing::TreeExpansionListener::PerlTreeExpansionListener->new(
            $object_name,
            $callbacks_name
        );
    ...
    no strict;
    $sender->addTreeExpansionListener($listener);
}
...
sub disconnect {
    ...
    $sender->removeTreeExpansionListener(
        $listeners{$object_name}{$callbacks_name}
    );
}

sub _TreeExpansionListener {
    ...
    my $methods = $callbacks{$sender_name}{$callbacks_name}
        or die "No registered _TreeExpansionListener callback for $sender_name\n";
