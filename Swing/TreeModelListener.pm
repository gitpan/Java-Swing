package Java::Swing::TreeModelListener;
use strict; use warnings;

use Inline Java      => 'STUDY',
           AUTOSTUDY => 1,
           STUDY     => ['PerlTreeModelListener'];
...
sub connect {
    ...
    my $listener       =
        Java::Swing::TreeModelListener::PerlTreeModelListener->new(
            $object_name,
            $callbacks_name
        );
    ...
    no strict;
    $sender->addTreeModelListener($listener);
}
...
sub disconnect {
    ...
    $sender->removeTreeModelListener(
        $listeners{$object_name}{$callbacks_name}
    );
}

sub _TreeModelListener {
    ...
    my $methods = $callbacks{$sender_name}{$callbacks_name}
        or die "No registered _TreeModelListener callback for $sender_name\n";
