package Java::Swing::ListSelectionListener;
use strict; use warnings;

use Inline Java      => 'STUDY',
           AUTOSTUDY => 1,
           STUDY     => ['PerlListSelectionListener'];
...
sub connect {
    ...
    my $listener       =
        Java::Swing::ListSelectionListener::PerlListSelectionListener->new(
            $object_name,
            $callbacks_name
        );
    ...
    no strict;
    $sender->addListSelectionListener($listener);
}
...
sub disconnect {
    ...
    $sender->removeListSelectionListener(
        $listeners{$object_name}{$callbacks_name}
    );
}

sub _ListSelectionListener {
    ...
    my $methods = $callbacks{$sender_name}{$callbacks_name}
        or die "No registered _ListSelectionListener callback for $sender_name\n";
