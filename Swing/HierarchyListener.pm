package Java::Swing::HierarchyListener;
use strict; use warnings;

use Inline Java      => 'STUDY',
           AUTOSTUDY => 1,
           STUDY     => ['PerlHierarchyListener'];
...
sub connect {
    ...
    my $listener       =
        Java::Swing::HierarchyListener::PerlHierarchyListener->new(
            $object_name,
            $callbacks_name
        );
    ...
    no strict;
    $sender->addHierarchyListener($listener);
}
...
sub disconnect {
    ...
    $sender->removeHierarchyListener(
        $listeners{$object_name}{$callbacks_name}
    );
}

sub _HierarchyListener {
    ...
    my $methods = $callbacks{$sender_name}{$callbacks_name}
        or die "No registered _HierarchyListener callback for $sender_name\n";
