package Java::Swing::AWTEventListener;
use strict; use warnings;

use Inline Java      => 'STUDY',
           AUTOSTUDY => 1,
           STUDY     => ['PerlAWTEventListener'];
...
sub connect {
    ...
    my $listener       =
        Java::Swing::AWTEventListener::PerlAWTEventListener->new(
            $object_name,
            $callbacks_name
        );
    ...
    no strict;
    $sender->addAWTEventListener($listener);
}
...
sub disconnect {
    ...
    $sender->removeAWTEventListener(
        $listeners{$object_name}{$callbacks_name}
    );
}

sub _AWTEventListener {
    ...
    my $methods = $callbacks{$sender_name}{$callbacks_name}
        or die "No registered _AWTEventListener callback for $sender_name\n";
