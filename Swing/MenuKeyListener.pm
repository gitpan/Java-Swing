package Java::Swing::MenuKeyListener;
use strict; use warnings;

use Inline Java      => 'STUDY',
           AUTOSTUDY => 1,
           STUDY     => ['PerlMenuKeyListener'];
...
sub connect {
    ...
    my $listener       =
        Java::Swing::MenuKeyListener::PerlMenuKeyListener->new(
            $object_name,
            $callbacks_name
        );
    ...
    no strict;
    $sender->addMenuKeyListener($listener);
}
...
sub disconnect {
    ...
    $sender->removeMenuKeyListener(
        $listeners{$object_name}{$callbacks_name}
    );
}

sub _MenuKeyListener {
    ...
    my $methods = $callbacks{$sender_name}{$callbacks_name}
        or die "No registered _MenuKeyListener callback for $sender_name\n";
