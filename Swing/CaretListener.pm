package Java::Swing::CaretListener;
use strict; use warnings;

use Inline Java      => 'STUDY',
           AUTOSTUDY => 1,
           STUDY     => ['PerlCaretListener'];
...
sub connect {
    ...
    my $listener       =
        Java::Swing::CaretListener::PerlCaretListener->new(
            $object_name,
            $callbacks_name
        );
    ...
    no strict;
    $sender->addCaretListener($listener);
}
...
sub _CaretListener {
    ...
    my $methods = $callbacks{$sender_name}{$callbacks_name}
        or die "No registered _CaretListener callback for $sender_name\n";
