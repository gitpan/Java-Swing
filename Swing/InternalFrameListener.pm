package Java::Swing::InternalFrameListener;
use strict; use warnings;

use Inline Java      => 'STUDY',
           AUTOSTUDY => 1,
           STUDY     => ['PerlInternalFrameListener'];
...
sub connect {
    ...
    my $listener       =
        Java::Swing::InternalFrameListener::PerlInternalFrameListener->new(
            $object_name,
            $callbacks_name
        );
    ...
    no strict;
    $sender->addInternalFrameListener($listener);
}
...
sub disconnect {
    ...
    $sender->removeInternalFrameListener(
        $listeners{$object_name}{$callbacks_name}
    );
}

sub _InternalFrameListener {
    ...
    my $methods = $callbacks{$sender_name}{$callbacks_name}
        or die "No registered _InternalFrameListener callback for $sender_name\n";
