package Java::Swing::MenuListener;
use strict; use warnings;

use Inline Java      => 'STUDY',
           AUTOSTUDY => 1,
           STUDY     => ['PerlMenuListener'];
...
sub connect {
    ...
    my $listener       =
        Java::Swing::MenuListener::PerlMenuListener->new(
            $object_name,
            $callbacks_name
        );
    ...
    no strict;
    $sender->addMenuListener($listener);
}
...
sub _MenuListener {
    ...
    my $methods = $callbacks{$sender_name}{$callbacks_name}
        or die "No registered _MenuListener callback for $sender_name\n";
