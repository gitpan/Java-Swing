package Java::Swing::FocusListener;
use strict; use warnings;

use Inline Java      => 'STUDY',
           AUTOSTUDY => 1,
           STUDY     => ['PerlFocusListener'];
...
sub connect {
    ...
    my $listener       =
        Java::Swing::FocusListener::PerlFocusListener->new(
            $object_name,
            $callbacks_name
        );
    ...
    no strict;
    $sender->addFocusListener($listener);
}
...
sub _FocusListener {
    ...
    my $methods = $callbacks{$sender_name}{$callbacks_name}
        or die "No registered _FocusListener callback for $sender_name\n";
