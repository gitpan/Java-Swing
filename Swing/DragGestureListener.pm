package Java::Swing::DragGestureListener;
use strict; use warnings;

use Inline Java      => 'STUDY',
           AUTOSTUDY => 1,
           STUDY     => ['PerlDragGestureListener'];
...
sub connect {
    ...
    my $listener       =
        Java::Swing::DragGestureListener::PerlDragGestureListener->new(
            $object_name,
            $callbacks_name
        );
    ...
    no strict;
    $sender->addDragGestureListener($listener);
}
...
sub disconnect {
    ...
    $sender->removeDragGestureListener(
        $listeners{$object_name}{$callbacks_name}
    );
}

sub _DragGestureListener {
    ...
    my $methods = $callbacks{$sender_name}{$callbacks_name}
        or die "No registered _DragGestureListener callback for $sender_name\n";
