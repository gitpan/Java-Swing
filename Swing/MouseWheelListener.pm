package Java::Swing::MouseWheelListener;
use strict; use warnings;

use Inline Java      => 'STUDY',
           AUTOSTUDY => 1,
           STUDY     => ['PerlMouseWheelListener'];
...
sub connect {
    ...
    my $listener       =
        Java::Swing::MouseWheelListener::PerlMouseWheelListener->new(
            $object_name,
            $callbacks_name
        );
    ...
    no strict;
    $sender->addMouseWheelListener($listener);
}
...
sub disconnect {
    ...
    $sender->removeMouseWheelListener(
        $listeners{$object_name}{$callbacks_name}
    );
}

sub _MouseWheelListener {
    ...
    my $methods = $callbacks{$sender_name}{$callbacks_name}
        or die "No registered _MouseWheelListener callback for $sender_name\n";
