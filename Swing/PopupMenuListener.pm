package Java::Swing::PopupMenuListener;
use strict; use warnings;

use Inline Java      => 'STUDY',
           AUTOSTUDY => 1,
           STUDY     => ['PerlPopupMenuListener'];
...
sub connect {
    ...
    my $listener       =
        Java::Swing::PopupMenuListener::PerlPopupMenuListener->new(
            $object_name,
            $callbacks_name
        );
    ...
    no strict;
    $sender->addPopupMenuListener($listener);
}
...
sub disconnect {
    ...
    $sender->removePopupMenuListener(
        $listeners{$object_name}{$callbacks_name}
    );
}

sub _PopupMenuListener {
    ...
    my $methods = $callbacks{$sender_name}{$callbacks_name}
        or die "No registered _PopupMenuListener callback for $sender_name\n";
