package Java::Swing::HierarchyBoundsListener;
use strict; use warnings;

use Inline Java      => 'STUDY',
           AUTOSTUDY => 1,
           STUDY     => ['PerlHierarchyBoundsListener'];
...
sub connect {
    ...
    my $listener       =
        Java::Swing::HierarchyBoundsListener::PerlHierarchyBoundsListener->new(
            $object_name,
            $callbacks_name
        );
    ...
    no strict;
    $sender->addHierarchyBoundsListener($listener);
}
...
sub disconnect {
    ...
    $sender->removeHierarchyBoundsListener(
        $listeners{$object_name}{$callbacks_name}
    );
}

sub _HierarchyBoundsListener {
    ...
    my $methods = $callbacks{$sender_name}{$callbacks_name}
        or die "No registered _HierarchyBoundsListener callback for $sender_name\n";
