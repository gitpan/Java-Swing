package Java::Swing::ItemListener;
use strict; use warnings;

use Inline Java      => 'STUDY',
           AUTOSTUDY => 1,
           STUDY     => ['PerlItemListener'];
...
sub connect {
    ...
    my $listener       =
        Java::Swing::ItemListener::PerlItemListener->new(
            $object_name,
            $callbacks_name
        );
    ...
    no strict;
    $sender->addItemListener($listener);
}
...
sub _ItemListener {
    ...
    my $methods = $callbacks{$sender_name}{$callbacks_name}
        or die "No registered _ItemListener callback for $sender_name\n";
