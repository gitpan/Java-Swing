package Java::Swing::UndoableEditListener;
use strict; use warnings;

use Inline Java      => 'STUDY',
           AUTOSTUDY => 1,
           STUDY     => ['PerlUndoableEditListener'];
...
sub connect {
    ...
    my $listener       =
        Java::Swing::UndoableEditListener::PerlUndoableEditListener->new(
            $object_name,
            $callbacks_name
        );
    ...
    no strict;
    $sender->addUndoableEditListener($listener);
}
...
sub disconnect {
    ...
    $sender->removeUndoableEditListener(
        $listeners{$object_name}{$callbacks_name}
    );
}

sub _UndoableEditListener {
    ...
    my $methods = $callbacks{$sender_name}{$callbacks_name}
        or die "No registered _UndoableEditListener callback for $sender_name\n";
