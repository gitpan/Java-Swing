package Java::Swing::WindowFocusListener;
use strict; use warnings;

use Inline Java      => 'STUDY',
           AUTOSTUDY => 1,
           STUDY     => ['PerlWindowFocusListener'];
...
sub connect {
    ...
    my $listener       =
        Java::Swing::WindowFocusListener::PerlWindowFocusListener->new(
            $object_name,
            $callbacks_name
        );
    ...
    no strict;
    $sender->addWindowFocusListener($listener);
}
...
sub disconnect {
    ...
    $sender->removeWindowFocusListener(
        $listeners{$object_name}{$callbacks_name}
    );
}

sub _WindowFocusListener {
    ...
    my $methods = $callbacks{$sender_name}{$callbacks_name}
        or die "No registered _WindowFocusListener callback for $sender_name\n";
