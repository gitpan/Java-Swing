package Java::Swing::InputMethodListener;
use strict; use warnings;

use Inline Java      => 'STUDY',
           AUTOSTUDY => 1,
           STUDY     => ['PerlInputMethodListener'];
...
sub connect {
    ...
    my $listener       =
        Java::Swing::InputMethodListener::PerlInputMethodListener->new(
            $object_name,
            $callbacks_name
        );
    ...
    no strict;
    $sender->addInputMethodListener($listener);
}
...
sub disconnect {
    ...
    $sender->removeInputMethodListener(
        $listeners{$object_name}{$callbacks_name}
    );
}

sub _InputMethodListener {
    ...
    my $methods = $callbacks{$sender_name}{$callbacks_name}
        or die "No registered _InputMethodListener callback for $sender_name\n";
