package Java::Swing::CellEditorListener;
use strict; use warnings;

use Inline Java      => 'STUDY',
           AUTOSTUDY => 1,
           STUDY     => ['PerlCellEditorListener'];
...
sub connect {
    ...
    my $listener       =
        Java::Swing::CellEditorListener::PerlCellEditorListener->new(
            $object_name,
            $callbacks_name
        );
    ...
    no strict;
    $sender->addCellEditorListener($listener);
}
...
sub disconnect {
    ...
    $sender->removeCellEditorListener(
        $listeners{$object_name}{$callbacks_name}
    );
}

sub _CellEditorListener {
    ...
    my $methods = $callbacks{$sender_name}{$callbacks_name}
        or die "No registered _CellEditorListener callback for $sender_name\n";
