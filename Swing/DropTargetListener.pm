package Java::Swing::DropTargetListener;
use strict; use warnings;

use Inline Java      => 'STUDY',
           AUTOSTUDY => 1,
           STUDY     => ['PerlDropTargetListener'];
...
sub connect {
    ...
    my $listener       =
        Java::Swing::DropTargetListener::PerlDropTargetListener->new(
            $object_name,
            $callbacks_name
        );
    ...
    no strict;
    $sender->addDropTargetListener($listener);
}
...
sub disconnect {
    ...
    $sender->removeDropTargetListener(
        $listeners{$object_name}{$callbacks_name}
    );
}

sub _DropTargetListener {
    ...
    my $methods = $callbacks{$sender_name}{$callbacks_name}
        or die "No registered _DropTargetListener callback for $sender_name\n";
