package Java::Swing::AncestorListener;
use strict; use warnings;

use Inline Java      => 'STUDY',
           AUTOSTUDY => 1,
           STUDY     => ['PerlAncestorListener'];
...
sub connect {
    ...
    my $listener       =
        Java::Swing::AncestorListener::PerlAncestorListener->new(
            $object_name,
            $callbacks_name
        );
    ...
    no strict;
    $sender->addAncestorListener($listener);
}
...
sub disconnect {
    ...
    $sender->removeAncestorListener(
        $listeners{$object_name}{$callbacks_name}
    );
}

sub _AncestorListener {
    ...
    my $methods = $callbacks{$sender_name}{$callbacks_name}
        or die "No registered _AncestorListener callback for $sender_name\n";
