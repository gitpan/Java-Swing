package Java::Swing::DragSourceListener;
use strict; use warnings;

use Inline Java      => 'STUDY',
           AUTOSTUDY => 1,
           STUDY     => ['PerlDragSourceListener'];
...
sub connect {
    ...
    my $listener       =
        Java::Swing::DragSourceListener::PerlDragSourceListener->new(
            $object_name,
            $callbacks_name
        );
    ...
    no strict;
    $sender->addDragSourceListener($listener);
}
...
sub disconnect {
    ...
    $sender->removeDragSourceListener(
        $listeners{$object_name}{$callbacks_name}
    );
}

sub _DragSourceListener {
    ...
    my $methods = $callbacks{$sender_name}{$callbacks_name}
        or die "No registered _DragSourceListener callback for $sender_name\n";
