package Java::Swing::TableModelListener;
use strict; use warnings;

use Inline Java      => 'STUDY',
           AUTOSTUDY => 1,
           STUDY     => ['PerlTableModelListener'];
...
sub connect {
    ...
    my $listener       =
        Java::Swing::TableModelListener::PerlTableModelListener->new(
            $object_name,
            $callbacks_name
        );
    ...
    no strict;
    $sender->addTableModelListener($listener);
}
...
sub disconnect {
    ...
    $sender->removeTableModelListener(
        $listeners{$object_name}{$callbacks_name}
    );
}

sub _TableModelListener {
    ...
    my $methods = $callbacks{$sender_name}{$callbacks_name}
        or die "No registered _TableModelListener callback for $sender_name\n";
