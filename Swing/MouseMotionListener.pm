package Java::Swing::MouseMotionListener;
use strict; use warnings;

use Inline Java      => 'STUDY',
           AUTOSTUDY => 1,
           STUDY     => ['PerlMouseMotionListener'];
...
sub connect {
    ...
    my $listener       =
        Java::Swing::MouseMotionListener::PerlMouseMotionListener->new(
            $object_name,
            $callbacks_name
        );
    ...
    no strict;
    $sender->addMouseMotionListener($listener);
}
...
sub disconnect {
    ...
    $sender->removeMouseMotionListener(
        $listeners{$object_name}{$callbacks_name}
    );
}

sub _MouseMotionListener {
    ...
    my $methods = $callbacks{$sender_name}{$callbacks_name}
        or die "No registered _MouseMotionListener callback for $sender_name\n";
