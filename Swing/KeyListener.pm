package Java::Swing::KeyListener;
use strict; use warnings;

use Inline Java      => 'STUDY',
           AUTOSTUDY => 1,
           STUDY     => ['PerlKeyListener'];
...
sub connect {
    ...
    my $listener       =
        Java::Swing::KeyListener::PerlKeyListener->new(
            $object_name,
            $callbacks_name
        );
    ...
    no strict;
    $sender->addKeyListener($listener);
}
...
sub _KeyListener {
    ...
    my $methods = $callbacks{$sender_name}{$callbacks_name}
        or die "No registered _KeyListener callback for $sender_name\n";
