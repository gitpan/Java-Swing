package Java::Swing::TableColumnModelListener;
use strict; use warnings;

use Inline Java      => 'STUDY',
           AUTOSTUDY => 1,
           STUDY     => ['PerlTableColumnModelListener'];
...
sub connect {
    ...
    my $listener       =
        Java::Swing::TableColumnModelListener::PerlTableColumnModelListener->new(
            $object_name,
            $callbacks_name
        );
    ...
    no strict;
    $sender->addTableColumnModelListener($listener);
}
...
sub disconnect {
    ...
    $sender->removeTableColumnModelListener(
        $listeners{$object_name}{$callbacks_name}
    );
}

sub _TableColumnModelListener {
    ...
    my $methods = $callbacks{$sender_name}{$callbacks_name}
        or die "No registered _TableColumnModelListener callback for $sender_name\n";
