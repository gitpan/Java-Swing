package Java::Swing::HyperlinkListener;
use strict; use warnings;

use Inline Java      => 'STUDY',
           AUTOSTUDY => 1,
           STUDY     => ['PerlHyperlinkListener'];
...
sub connect {
    ...
    my $listener       =
        Java::Swing::HyperlinkListener::PerlHyperlinkListener->new(
            $object_name,
            $callbacks_name
        );
    ...
    no strict;
    $sender->addHyperlinkListener($listener);
}
...
sub disconnect {
    ...
    $sender->removeHyperlinkListener(
        $listeners{$object_name}{$callbacks_name}
    );
}

sub _HyperlinkListener {
    ...
    my $methods = $callbacks{$sender_name}{$callbacks_name}
        or die "No registered _HyperlinkListener callback for $sender_name\n";
