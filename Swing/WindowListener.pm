package Java::Swing::WindowListener;
use strict; use warnings;

use Inline Java      => 'STUDY',
           AUTOSTUDY => 1,
           STUDY     => ['PerlWindowListener'];
...
sub connect {
    ...
    my $listener       =
        Java::Swing::WindowListener::PerlWindowListener->new(
            $object_name,
            $callbacks_name
        );
    ...
    no strict;
    $sender->addWindowListener($listener);
}
...
sub _WindowListener {
    ...
    my $methods = $callbacks{$sender_name}{$callbacks_name}
        or die "No registered _WindowListener callback for $sender_name\n";
